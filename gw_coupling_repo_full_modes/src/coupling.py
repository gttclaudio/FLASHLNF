...
def extract_fields_from_data(df_raw):
    ...
    return coords, Es, volume_weights, denominator


def coupling(integrand_fn, denominator, volume_weights, *args, **kwargs):
    integrands = integrand_fn(*args, **kwargs)

    if not isinstance(integrands, (list, tuple)):
        integrands = [integrands]

    results = []

    for g in integrands:
        I = np.sum(volume_weights * g)
        results.append(np.abs(I)**2 / denominator)
...
def gw_tt_integrand(beta, phi, coords, Es, freq_hz):
    x, y, z = coords
    ex, ey, ez = Es
    ...
    propagation_coordinate = sin_beta * y_phi + cos_beta * z
    tt_phase = phase(propagation_coordinate, wave_number)

    j_eff_x = sin_beta * tt_phase
    j_eff_y = sin_beta * cos_beta * tt_phase
    j_eff_z = sin_beta**2 * tt_phase

    parallel = ex_phi * j_eff_x
    cross = ey_phi * j_eff_y - ez * j_eff_z

    return [parallel, cross]


def axion_integrand(Es):
    _, _, ez = Es

    return ez


def vector_integrand(beta, phi, Es):
    ex, ey, ez = Es
    sin_beta = np.sin(beta)
    cos_beta = np.cos(beta)
    ...
    return ey_phi * sin_beta + ez * cos_beta


def scalar_integrand(beta, phi, coords, Bs, wave_number):
    x, y, z = coords
    bx, by, bz = Bs
    ...
    propagation_coordinate = sin_beta * y_phi + cos_beta * z
    scalar_phase = phase(propagation_coordinate, wave_number)

    return bz * scalar_phase
...
def _worker_angle(task):
    beta, phi, integrand_fn, coupling_args, volume_weights, denominator = task

    return coupling(integrand_fn, denominator, volume_weights, beta, phi, *coupling_args)
...
def run_scan_for_csv(csv_path, coupling_type, z_min_mm=0.0, z_max_mm=1500.0, n_angles=201, n_processes=6, output_dir=None):
    ...
    coords, Es, volume_weights, denominator = extract_fields_from_data(df)

    if coupling_type == "axion":
        ...
        betas = np.linspace(0.0, np.pi, n_angles)
        ...
        if coupling_type == "gw":
            ...
            coupling_args = (coords, Es, freq_hz)
            integrand_fn = gw_tt_integrand

        elif coupling_type == "dp":

            label = "DP"

            coupling_args = (Es,)
            integrand_fn = vector_integrand

        else:
            raise ValueError("Invalid coupling type.")

        tasks = [
            (beta, phi, integrand_fn, coupling_args, volume_weights, denominator) for beta, phi in angle_pairs
        ]
        ...
        if coupling_type == "dp":
            ...
            records = [
                {"beta": beta, "phi": phi, "coupling": coupling}
                for (beta, phi), (coupling) in zip(angle_pairs, results)
            ]
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Compute coupling from cavity-field CSV files."
    )
    parser.add_argument(
        "--source",
        choices=["gw", "axion", "dp", "scalar"],
        default="gw",
        help="Choose which particle the coupling is computed for.",
    )
    parser.add_argument(
        "--mode",
...
if __name__ == "__main__":
    ...
    if args.csv:
        run_scan(csv_files=args.csv, coupling_type=args.source, **common_kwargs)
    elif args.mode:
        run_scan(csv_files=[resolve_mode_csv(args.mode, args.data_dir)], coupling_type=args.source, **common_kwargs)
    else:
        run_scan(
            folder=args.folder or args.data_dir,
            filename_filter=args.filename_filter,
            coupling_type=args.source,
            **common_kwargs,
        )
